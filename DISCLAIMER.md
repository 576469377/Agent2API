...
0.2. **本项目不是为生产环境设计的，也不是为了对外提供服务。** 代码以「把原理讲清楚」为优先，而非「扛住线上流量」。已知缺陷清单见 [README 的「已知限制」章节](README.md#已知限制)。
...
**已知缺陷清单并不完整**——详见 [README 的「已知限制」章节](README.md#已知限制)。该章节如实列出了当前代码中已确认存在、尚未修复的问题，请在使用前阅读。

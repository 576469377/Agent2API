...
- **启动横幅重排**：
  ...
  - 多账号横幅支持 ⏸「已停用」状态标记
  - （随工作区中的 doubao 平台接线一并提交：平台白名单扩展与平台工厂）
...
- **第二个平台：豆包 DoubaoWork**（`-platform doubao`，`internal/adapter/doubao`）：复用本机 DoubaoWork 桌面端的登录态，把豆包网页端转成同一套 OpenAI / Anthropic 接口。号池、控制台、三种下游协议零改动复用 —— 验证了「N 个平台 × M 个协议」的分层确实成立
  - **凭证**：直接解密客户端 Chromium profile 的 Cookie 库（钥匙串口令 → PBKDF2 → AES-CBC）。实测该客户端的字节布局是 `v10 + 16B + IV + 密文`，比常见的 `v10 + IV + 密文` 多一层，因此三种布局都试、取最长的可打印解（CBC 自同步会让偏移错误时解出「被截半但仍可打印」的结果）
  - **协议**（2026-09-17 实测）：`POST /samantha/chat/completion`；`conversation_id` 必填（空串直接 400），故拨号前先建会话、流结束后尽力删除；正文在 `event_type=2001` 帧的 `message.content`（**转义的 JSON 字符串**）里
  - **两个已知的降级**：没有 system 角色（系统提示折进首条用户消息）；不支持工具调用（工具结果降级为文本）
  - 新增 `agent2api login -upstream doubao -out auths/a.json` 导出 JSON 凭证，供多账号号池与无客户端的机器
  - 回归测试 13 例（请求构造 / 系统提示折叠 / 降级 / Cookie 三种布局解密 / 真实抓包样本的整条 SSE / 全量帧去重 / 意外中断收尾 / 错误帧 / 清理只执行一次）

...
- **限流不再盲目锁定账号**：调度策略从「限流即冷却（无精确时刻时按指数退避自造冷却时长）」改为「**失败换下一个，不锁定**」——上游未给出精确重置时刻的限流/额度耗尽（14018）失败只降健康度并尝试下一个账号，全部账号失败才返回真实错误；仅当上游给出**精确重置时刻**（如「将在 16:51 重置」）时才冷却对应 (账号, 模型) 格子。修复「一个问题问完，两个账号同一模型都被冷却（一个十几小时、一个几十秒）」的问题。401 鉴权失效仍保留整号 blocked 终态
...
- **限流与额度耗尽改为「默认不锁定」**：上游的 `credits` 字段是「积分倍率」而非账号余额，免费/低成本模型不消耗付费额度；且上游多数限流并不给出重置时刻。因此只有上游文案自带**精确重置时刻**时才冷却对应的 (账号, 模型)，其余限流与额度耗尽（14018）一律只降健康度并换号 —— 实测一次普通限流就能把两个号先后挂起（「一个问题问完，两个号都冷却」）。此前先后试过「模型级指数退避」与「账号级长冷却」，都会在无重置时刻时把可用容量整体锁死，均已废弃

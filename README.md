...
| 页面 | 内容 |
|---|---|
| **概览** | 状态墙、请求趋势图、Token 柱状图、协议/模型/**账号**排行、最近请求；带倒计时的自动刷新，切后台自动暂停 |
| **账号** | 号池管理：每个账号的调度状态（正常/限流冷却/鉴权失效/已停用）、登录状态与凭证有效期、**每账号用量**；支持启停、清除冷却、**设备码重新登录**、**添加账号** |
| **对话** | 流式输出、思考过程折叠、采样参数可调、代码块复制、支持表格与公式渲染 |
| **平台** | 上游地址、账号数、模型数、健康状态；规划中的平台 |
| **模型** | 模型清单与能力标签；可切换**账号 × 模型矩阵**，看每个账号支持哪些模型 |
| **调用日志** | 最近 200 条请求（含**实际使用的账号**），可按成功/失败过滤，失败行高亮 |
| **设置** | 脱敏开关（热更新）、重新拉取模型清单、网关访问密钥管理、当前生效配置 |
...
> [!NOTE]
> 核心功能零 CDN 依赖。仅**数学公式渲染**会在首次遇到时按需加载 KaTeX，失败或离线时降级为原始文本，不影响其余功能。截图中的账号为演示数据。
...
以上全部能力在**网页控制台的「账号」页**都有对应的可视化与操作：

- 每账号的调度状态（▸ 标出「下一个使用」）、登录状态与凭证有效期、每账号用量统计（落盘持久化，重启不清零）
- **停用/启用**：临时把某个账号摘出轮询（如怀疑它被上游风控），不用重启网关
- **清除冷却**：上游提前恢复时手动解冻，不用等倒计时
- **重新登录**：某账号掉线（refreshToken 过期）时，在页面上直接发起设备码登录，扫码/授权后凭证自动写回原文件
- **添加账号**：页面上点「添加账号」即可发起新账号的设备码登录，新凭证自动落入号池目录
- 调用日志每条记录标注实际使用的账号；模型页可切「账号 × 模型矩阵」查看各账号的模型差异
